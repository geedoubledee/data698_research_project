...
for (i in 1:length(fns)){
    if (!file.exists(fns[i])){
        col <- i + 1
        logm <- cv.glmnet(train_x, train_y[, col],
...
for (i in 1:length(logms)){
    logm <- logms[[i]]
    logm_pred <- predict(logm, validate_x, type = "class")
    logm_preds[[i]] <- logm_pred
    col <- i + 1
    logmcm_complete <- confusionMatrix(as.factor(logm_pred), validate_y[, col],
                                       positive = "Yes")
...
col1 <- as.data.frame(c(rep("Naive Bayes", 7), rep("Support Vector Machine", 7),
          rep("eXtreme Gradient Boosting", 7), rep("Logistic Regression", 7)))
colnames(col1) <- c("Model")
col2 <- as.data.frame(rep(prob, 4))
...
metrics <- as.data.frame(cbind(col1, col2, by_class, overall))
keep <- c("Model", "Problem", "Accuracy", "Kappa", "Precision",
