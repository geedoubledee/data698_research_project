...
During the data collection process, bad links were given a `Visited` value of -1 to separate them from good links we had already visited (value: 1) and links we had not yet visited (value: 0). Some of these bad links were added to the recipes data erroneously, so we identify and remove them. 
...
A small number of recipes that have never been reviewed also made their way into the data, so we remove those as well. 
...
We start by fuzzy clustering the reviews into topics using bigrams. Since some words that are commonly considered stopwords for text data might be useful for our analysis, we will not exclude bigrams that contain words like "too" and "not" from consideration. Many reviews will definitely include text like "too much" or "not enough," and we hope fuzzy clustering will group reviews together based on their similar language. We also eliminate bigrams containing numbers.
...
Using values of k from two to 12, we measure the perplexity of several different Latent Dirichlet Allocation (LDA) models that would automatically divide the data into k topics. 
...
The reduction in perplexity begins to drop off after about six topics, so we set k to six in the final LDA model.
...
We see a lot of overlap in the bigrams that are common to these six topics. In topics one, two, and six, "next time" is the most identifying bigram, indicating the reviews in each of these topics are likely to exhibit some disappointment in the results and some suggestions for improving those results in the future. Despite all the overlap, there are also bigrams that are unique to each topic. 

It's notable that some of these unique bigrams are related to specific ingredients, i.e. brown sugar being an identifier of topic one, cream cheese being an identifier of topic four, and soy sauce being an identifier of topic six. Ingredients have therefore resulted in some unintended grouping. We check for an additional instance of unintended grouping since the reviews are of various lengths, and it is reasonable to suspect review length might have incidentally factored into the topic decision boundaries. 
...
The topics actually have pretty similar review length distributions. Topic 4 has the highest median review length at 201 characters. We export the potentially negative reviews for manual labeling. 
...
We have labeled 3,398 observations in total: 515 observations from each of the six identified topics, plus 308 observations that were not composed of any significant bigrams and were thus assigned a topic of `NA`. (We have recoded their assigned topic as 0.) Reasons why a review might not be composed of any significant bigrams include:
...
Because we only looked at reviews with star ratings of 3 or less, we originally felt it should not be possible for these reviews to receive a "Positive" label for `SENTIMENT`. However, we did encounter review text along the way that was clearly neither "Negative" nor "Neutral," and it's possible the reviewer intended to leave a higher star rating along with their seemingly "Positive" words. For the sake of consistency, we continued to label these "Positive" reviews as "Neutral" so that we could redefine our `SENTIMENT` variable later and eliminate any confusion. To that end, we now replace the `SENTIMENT` variable with the `NEGATIVE` variable, a binary predictor in which a value of 1 indicates the review text expressed a "Negative" sentiment, and a value of 0 indicates it did not. 
...
We visualize the frequency of reviews we labeled `NEGATIVE` by star rating.
...
Most of the 1 and 2 star reviews we labeled expressed "Negative" sentiment, whereas most of the 3 star reviews we expressed did not. We also encountered more than twice as many 3 star reviews as we did 1 or 2 star reviews during labeling.

We visualize the frequency and some of the overlap of these problems. 
...
Reviews that identify `MEASUREMENTS` errors are most frequent, whereas reviews that identify `TEMPERATURE` errors are least frequent. When reviews mention two problems, `MEASUREMENTS` is frequently one of them. It is also worth noting that the various ingredient problems frequently occur together, as well as with authenticity problems. Lastly, `TEMPERATURE` errors occur frequently with `TIMING` errors, which is sensible because timing issues can often be solved with temperature changes and vice versa.
...
Since unsupervised learning was insufficient for our needs as anticipated, we now proceed with preparing the data further for supervised learning methods. 
...
ar_df_long_neg_labs_only <- ar_df_long_neg_labs_only_num |>
    mutate(across(all_of(prob), ~ factor(., levels = c(0,1),
                                         labels = c("No", "Yes"))))
ar_df_long_neg_bigrams_sparse <- tidy(ar_df_long_neg_bigrams_dtm) |>
    right_join(ar_df_long_neg_labs_only |> select(ID),
               by = join_by(document == ID)) |>
    arrange(document) |>
    mutate(term = ifelse(is.na(term), "***_***", term),
...
```{r }
set.seed(1006)
sample <- sample(nrow(ar_df_long_neg_bigrams_sparse),
                 round(nrow(ar_df_long_neg_bigrams_sparse) * 0.5),
                 replace = FALSE)
train_x <- ar_df_long_neg_bigrams_sparse[sample, ]
train_y <- ar_df_long_neg_labs_only[sample, ]
train_y_num <- ar_df_long_neg_labs_only_num[sample, ]
validate_test_x <- ar_df_long_neg_bigrams_sparse[-sample, ]
...
for (i in 1:length(fns)){
    if (!file.exists(fns[i])){
        ...
        svm_tune <- tune(svm, train.x = train_x, train.y = train_y[, col],
                         kernel = "radial", ranges = tune_grid,
                         tunecontrol = ctrl)
