...
All the topics actually have pretty similar review length distributions. We export the potentially negative reviews for manual labeling. 
...
We have labeled 1,598 observations as either Neutral or Negative in `SENTIMENT`, and we have categorized them by which of nine specific problems they identify (if any). These problems and descriptions of the criteria used to define whether a review identified them are outlined below:
...
```{r }
sel <- c("ID", prob)
ar_df_long_neg_labs_only <- ar_df_long_neg_labs |>
    unite("ID", URL, Review_Num, sep = "_Review_Num_") |>
    select(all_of(sel)) |>
    arrange(ID) |>
